...
public:
  ...
  inline void set_IPAddress(uint8_t * addr);
  inline void get_IPAddress(uint8_t * addr);
...
public:
  ...
  __GP_REGISTER16 (INTLEVEL,    0x0013);    // Interrupt Low Level Timer Register
  __GP_REGISTER8 (IR,     0x0015);    // Interrupt
  __GP_REGISTER8 (IMR,    0x0016);    // Interrupt Mask
  __GP_REGISTER8 (SIR,     0x0017);    // Interrupt
  __GP_REGISTER8 (SIMR,    0x0018);    // Interrupt Mask
  ...
  __GP_REGISTER8 (VERSION,     0x0039);    // PHY Configuration register, default value: 0b 1011 1xxx
...
public:
  ...
  __SOCKET_REGISTER8(SnRX_BUF_SIZE,   0x001E)    // RX Buf Size
  __SOCKET_REGISTER8(SnTX_BUF_SIZE,   0x001F)    // TX Buf Size
...
public:
  ...
  static inline void setSS()   {
       //10 pin - PORTB, 2 register
        PORTB &= ~(1 << 2);
  }
  static inline void resetSS() {
      // 10 pin - PORTB, 2 register
      PORTB |= (1 << 2);
  }
};
...
void W5500Class::get_IPAddress(uint8_t *_addr) {
  readSIPR(_addr);
}

void W5500Class::set_IPAddress(uint8_t *_addr) {
  writeSIPR(_addr);
}
